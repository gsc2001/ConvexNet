...
def main():
    ...
    train_loader = create_loader(train_dataset, (cfg.in_channels, cfg.img_size, cfg.img_size), cfg.batch_size,
                                 is_training=True)
    ...
    valid_loader = create_loader(valid_dataset, (cfg.in_channels, cfg.img_size, cfg.img_size), cfg.batch_size)

    model = mixer.MlpMixer(cfg)
    module = mixer.MixerModule(model, cfg.lr, cfg.weight_decay, cfg.lr_warmup_epochs, cfg.num_epochs,
                               cfg.mixup_strength)
    trainer = pl.Trainer(gradient_clip_val=1)
