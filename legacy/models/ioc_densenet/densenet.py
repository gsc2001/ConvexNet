...
def _bn_function_factory(norm, elu, conv):
    def bn_function(*inputs):
        concated_features = torch.cat(inputs, 1)
        bottleneck_output = conv(elu(norm(concated_features)))
...
class _DenseLayer(nn.Module):
    ...
    def forward(self, *prev_features):
        bn_function = _bn_function_factory(self.norm1, self.elu1, self.conv1)
...
class IOC_DenseNet(nn.Module):
    ...
    def __init__(self, growth_rate=12, block_config=(16, 16, 16), compression=0.5,
        ...
        super(IOC_DenseNet, self).__init__()
        ...
        if small_inputs:
            ...
            self.features.add_module('elu0', nn.ELU(inplace=True))
